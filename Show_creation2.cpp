...
template <typename FUNC>
pair<uint, uint> move(pair<uint, uint> begin, pair<uint, uint> end, uint Duration, uint Current_frame)
{
	--Duration;
	pair<uint, uint> ans;
	FUNC f;
	ans.first = begin.first + (end.first - begin.first) * f((double)Current_frame / Duration) + 0.5;
	ans.second = begin.second + (end.second - begin.second) * f((double)Current_frame / Duration) + 0.5;
	return ans;
}
namespace CONSTANT
{
...
int main()
{
	...
	{
		fig.draw(0, 0, WHITE, fig.Show_Creation<FUNCTION::smooth>(45u, frame), 5u);
		char temp[64];
		sprintf(temp, "Show_Creation_Rectangle2_%d.bmp", frame+1);
		to_file(temp);
		printf("\r%.0lf%%", frame/0.9);
		fflush(stdout);
		fig.draw(0, 0, BLACK, fig.Show_Creation<FUNCTION::smooth>(45u, frame), 5u);
	}
	...
	//system("ffmpeg -y -i Show_Creation_Rectangle2_%d.bmp -r 30 -pix_fmt yuv420p Show_Creation_Rectangle2.mp4");
	printf("\n%ldms\n", clock() - st);
	return 0;
}
