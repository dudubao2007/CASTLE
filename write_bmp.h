...
void write_bits(uc *begin, uc *end, uc num)
{
...
uint write_bmp(uc *res, uint Height, uint Width, uc bpp, uc *BitMap, uint *ColorMap = NULL)
{
	//FILEHEADER
	uc *ptr1 = res;
	...
	write_bits(ptr2+20, ptr2+24, ((((bpp * (ull)Width) + 31) >> 5) << 2) * Height);
