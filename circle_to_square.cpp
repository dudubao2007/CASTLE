...
{
	return Point(xmin + (xmax - xmin) * b / (Width - 1), ymin + (ymax - ymin) * a / (Height - 1));
}
...
void get_sdf2(uint a, uint b)
{
	Point P = to_Point(a, b);
	double x = P.x, y = P.y;
	if (-x<=y&&y<=x&&-1.0<=y&&y<=1.0) sdf2[a][b] = x - 1.0;
	else if (-y<=x&&x<=y&&-1.0<=x&&x<=1.0) sdf2[a][b] = y - 1.0;
	else if (x<=y&&y<=-x&&-1.0<=y&&y<=1.0) sdf2[a][b] = -1.0 - x;
	else if (y<=x&&x<=-y&&-1.0<=x&&x<=1.0) sdf2[a][b] = -1.0 - y;
	else if (x>=1.0&&y>=1.0) sdf2[a][b] = hypot(x-1.0, y-1.0);
	else if (x<=-1.0&&y>=1.0) sdf2[a][b] = hypot(x+1.0, y-1.0);
	else if (x<=-1.0&&y<=-1.0) sdf2[a][b] = hypot(x+1.0, y+1.0);
	else if (x>=1.0&&y<=-1.0) sdf2[a][b] = hypot(x-1.0, y+1.0);
	else assert(0);
...
void get_sdf()
{
	for (uint i = 0;i < Height;++i)
		for (uint j = 0;j < Width;++j)
			sdf[i][j] = sdf1[i][j] + (sdf2[i][j] - sdf1[i][j]) * (frame - 30) / 89.0;
}
...
COLOR get_color()
{
	return COLOR(255.0 * (1.0 - (frame - 30) / 89.0) + 0.5, 255.0 * (frame - 30) / 89.0 + 0.5, 255);
}
...
int main()
{
	...
	for (frame = 0;frame < 30u;++frame)
	{
		...
		for (uint i = 0;i < Height;++i)
			for (uint j = 0;j < Width;++j)
				if (sdf1[i][j] <= 0.0) set_color(BitMap[i][j], c);
				else set_color(BitMap[i][j], b);
		char filename[64];
	...
	for (frame = 30;frame < 120u;++frame)
	{
		...
		for (uint i = 0;i < Height;++i)
			for (uint j = 0;j < Width;++j)
				if (sdf[i][j] <= 0.0) set_color(BitMap[i][j], c);
				else set_color(BitMap[i][j], b);
		char filename[64];
		sprintf(filename, "circle_to_square%d.bmp", (int)frame + 1);
		uint size = write_bmp(BMPFILE, Height, Width, 24, &BitMap[0][0][0]);
	...
	for (frame = 120;frame < 150u;++frame)
	{
		...
		for (uint i = 0;i < Height;++i)
			for (uint j = 0;j < Width;++j)
				if (sdf2[i][j] <= 0.0) set_color(BitMap[i][j], c);
				else set_color(BitMap[i][j], b);
		char filename[64];
		sprintf(filename, "circle_to_square%d.bmp", (int)frame + 1);
		uint size = write_bmp(BMPFILE, Height, Width, 24, &BitMap[0][0][0]);
	...
	system("ffmpeg -y -i circle_to_square%d.bmp -pix_fmt yuv420p -r 30 circle_to_square.mp4");
	return 0;
}
