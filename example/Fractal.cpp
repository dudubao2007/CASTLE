...
#include "castle.hpp"
using namespace std;
Axis axis = Axis(1024, 1024, 2048, 2048);
Float helper(Point a, Dword n)
{
	...
	{
		return static_cast<Float>(min(min(a.x - 1.0, 2.0 - a.x), min(a.y - 1.0, 2.0 - a.y)));
	}
	while (a.x >= 1.0) a.x -= 1.0;
	while (a.y >= 1.0) a.y -= 1.0;
...
{
	if (sdf(axis.to_point(i, j)) <= 2.0 / axis.unit()) return Color(0, 0, 0);
